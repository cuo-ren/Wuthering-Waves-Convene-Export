...
json validate_data() {
	...
	for (auto& [uid, value] : gacha_list.items()) {
		...
		//校验uid后是否为字典
		if (!value.is_object()) {
			std::cerr << "UID值类型错误，不是字典" << std::endl;
			json error2 = {
				{"code",2},
				{"data",{{"uid",uid}}}
			};
	...
	for (auto& [uid, value] : gacha_list.items()) {
		...
		if (!value.contains("info") or !value.contains("data") or value.size() != 2) {
			std::cerr << "UID中不包含info或data或数量错误" << std::endl;
			json error3 = {
				{"code",3},
				{"data",{{"uid",uid}}}
			};
		...
		if (!value["info"].contains("lang") or !value["info"].contains("update_time") or value["info"].size() != 2) {
			std::cerr << "info中缺失字段或数量错误" << std::endl;
			json error5 = {
				{"code",5},
				{"data",{{"uid",uid}}}
			};
		...
		if (!value["info"]["update_time"].is_number_integer()) {
			std::cerr << "update_time类型错误" << std::endl;
			json error6 = {
				{"code",6},
				{"data",{{"uid",uid}}}
			};
		...
		//校验lang字符串
		//这里的语言代码要修改
		std::vector<std::string> support_languages = { "zh-Hans","en" };
